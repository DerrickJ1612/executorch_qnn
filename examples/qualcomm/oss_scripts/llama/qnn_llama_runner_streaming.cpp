...
DEFINE_bool(jsonl,
    false,
    "When --stream, emit one JSON line per chunk");
...
void start_runner(
    ...
    // };
    auto callback = [&](const std::string& piece) {
        // keep legacy accumulation
        buf.insert(buf.end(), piece.begin(), piece.end());

        // live stream to stdout if requested
        if (FLAGS_stream) {
          if (FLAGS_jsonl) {
            std::cout << "{\"token\":\"" << escape_json(piece) << "\"}" << std::endl; // line-buffered
          } else {
            std::cout << piece << std::flush;  // raw text
          }
        }

        // write-through to file incrementally (so CI still gets output even if interrupted)
        if (fout.is_open()) {
          fout.write(piece.data(), static_cast<std::streamsize>(piece.size()));
          fout.flush();
        }
    };

  executorch::extension::llm::GenerationConfig config{
      true,                   // stream from runner (fires callback repeatedly)
      -1,
      false,
      FLAGS_seq_len,
  ...
  if (!fout.is_open()) {
    std::ofstream f2(FLAGS_output_path.c_str());
    f2.write(buf.data(), static_cast<std::streamsize>(buf.size()));
    f2.close();
  }
  if (fout.is_open()) {
      fout.close();
